...
def _load_faq_data() -> List[Dict]:
 ...
 print(f"[FAQ_TOOL] ERROR: FAQ file not found at {faq_file}")
 logger.error(f"[FAQ_TOOL] ERROR: FAQ file not found at {faq_file}")
 ...
 print(f"[FAQ_TOOL] ERROR loading FAQ file: {e}")
 logger.error(f"[FAQ_TOOL] ERROR loading FAQ file: {e}")
 import traceback
 traceback.print_exc()
 return []
...
def search_faq(query: str, use_semantic: bool = True) -> Dict[str, any]:
 ...
 if cached_result:
 print(f"[FAQ_TOOL] Cache invalidated for refund/policy query, doing fresh search")
 logger.info(f"[FAQ_TOOL] Cache invalidated for refund/policy query, doing fresh search")
 ...
 # Cache and return semantic result
 faq_cache.set(cache_key, result)
 print(f"[FAQ_TOOL] Returning PARTIAL from semantic search (similarity {similarity:.3f} >= 0.3 but < {threshold})")
 logger.info(f"[FAQ_TOOL] Returning PARTIAL from semantic search (similarity {similarity:.3f} >= 0.3 but < {threshold})")
 return result
 else:
 print(f"[FAQ_TOOL] Semantic search similarity {similarity:.3f} < 0.3, falling back to keyword search")
 logger.info(f"[FAQ_TOOL] Semantic search similarity {similarity:.3f} < 0.3, falling back to keyword search")
 # If similarity < 0.4, fall through to keyword search for better results
 else:
 print(f"[FAQ_TOOL] Semantic search returned no results, falling back to keyword search")
 logger.warning(f"[FAQ_TOOL] Semantic search returned no results, falling back to keyword search")
 ...
 print(f"[FAQ_TOOL] No FAQ matches found for query: '{query_lower}'")
