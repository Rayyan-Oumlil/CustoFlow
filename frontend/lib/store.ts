...
export const useStore = create<StoreState>((set) => ({
  ...
  setTheme: (theme) => {
    set({ theme })
    if (typeof window !== "undefined") {
      localStorage.setItem("cf-mode", theme)
  ...
  initFromStorage: () => {
    ...
    const savedTheme = (localStorage.getItem("cf-mode") as "light" | "dark") || "light"
    localStorage.setItem("custoflow_user_id", savedUserId)
    document.documentElement.dataset.mode = savedTheme
    set({ userId: savedUserId, customerId: normalizedCustomerId, theme: savedTheme })
  },
