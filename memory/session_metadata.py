...
class SessionMetadata:
 ...
 closed_count = 0
 if not tickets:
 print(f" No tickets found for session {session_id}")
 else:
 print(f" Found {len(tickets)} ticket(s) for session {session_id}")
 for ticket in tickets:
 ticket_id = ticket.get("ticket_id")
 current_status = ticket.get("status", "").lower() if ticket.get("status") else ""
 print(f" Ticket {ticket_id} current status: {current_status}")
 ...
 print(f" Ticket {ticket_id} already closed/resolved, skipping")
 if closed_count > 0:
 print(f" Closed {closed_count} ticket(s) associated with session {session_id}")
 else:
 print(f" No tickets needed to be closed for session {session_id}")
 except Exception as e:
 ...
 print(f" Session {session_id} not found in Supabase (may have been already deleted). Related data cleaned up.")
 return True
 except Exception as e:
 ...
 print(f" No tickets found in JSON mode for session {session_id}")
 else:
 print(f" Found {len(session_tickets)} ticket(s) for session {session_id} in JSON mode")
 ...
 # This is idempotent - return True even if session was already deleted
 print(f" Session {session_id} not found in JSON (may have been already deleted). Related data cleaned up.")
 return True
